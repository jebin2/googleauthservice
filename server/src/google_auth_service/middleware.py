...
def create_auth_middleware(
    ...
    This creates the base middleware logic. You'll need to wrap it
    in your framework's middleware pattern.
    
    Args:
        user_loader: Async function that takes user_id and returns user or None
        jwt_secret: Secret key for JWT verification
        route_config: RouteConfig with required/optional/public patterns
        jwt_algorithm: JWT algorithm (default: HS256)
        jwt_access_expiry_minutes: Access token expiry
        jwt_refresh_expiry_days: Refresh token expiry
        token_version_getter: Function to get token version from user object
        admin_checker: Function to check if user is admin
        admin_emails: List of admin email addresses
    
    Returns:
        AuthMiddlewareBase instance
    ...
    jwt_service = JWTService(
        ...
        access_expiry_minutes=jwt_access_expiry_minutes,
        refresh_expiry_days=jwt_refresh_expiry_days,
    )
    ...
    return AuthMiddlewareBase(
        ...
        token_version_getter=token_version_getter,
